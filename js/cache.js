...
layui.use(['form', 'jquery', "layer"], function () {
    ...
    function lockPage() {
        layer.open({
            ...
            '<div class="admin-header-lock-name" id="lockUserName">您好，' + $.cookie("truename") + '，已为您锁屏</div>' +
            '<div class="input_btn">' +
            '<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入手机号解锁.." name="lockPwd" id="lockPwd" />' +
            '<button class="layui-btn" id="unlock">解锁</button>' +
            '</div>' +
            '<p>请输入您的手机号，否则不会解锁成功哦！！！</p>' +
            '</div>',
            closeBtn: 0,
            shade: 0.9,
            success: function () {
    ...
    $("body").on("click", "#unlock", function () {
        if ($(this).siblings(".admin-header-lock-input").val() == '') {
            ...
            if ($(this).siblings(".admin-header-lock-input").val() == phone) {
